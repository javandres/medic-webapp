...
describe('audit proxy', () => {
  ...
  it('audit audits the request', done => {
    ...
    const proxy = {
      web: function(req) {
        chai.expect(Buffer.byteLength(JSON.stringify(auditedDoc)))
  ...
  it('audit does not audit non json request', done => {
    const target = 'http://localhost:4444';
    const username = 'steve';
    const doc = 'message_id=15095&sent_timestamp=1396224953456&message=ANCR+jessiec+18+18&from=%2B13125551212';
    const proxy = {
      web: function() {}
    };
    const passStreamFn = function(writeFn, endFn) {
  ...
  it('audit audits the non _local docs', done => {
    ...
    const proxy = {
      web: function(req) {
        chai.expect(Buffer.byteLength(JSON.stringify(docs)))
  ...
  it('audit emits errors when stream emits errors', done => {
    const target = 'http://localhost:4444';
    const generatedId = 'abc';
    const username = 'steve';
    const doc = {
    ...
    let writeFn,
        endFn;
    const passStreamFn = function(_writeFn, _endFn) {
      writeFn = _writeFn;
      endFn = _endFn;
    };
    const req = {
        ...
        return {
          on: function(eventName, callback) {
            if (eventName === 'error') {
              ...
              endFn.call({push: function(body) {
                chai.expect(body).to.equal(JSON.stringify(auditedDoc));
              }}, function(err) {
                if (err) {
                  callback(err);
                }
